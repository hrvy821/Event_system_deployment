...
export default function Login() {
  ...
  const [socialAuthReady, setSocialAuthReady] = useState({ google: null, facebook: null });
  ...
  useEffect(() => {
    ...
    api.get('/auth/google/status')
      .then((res) => {
        if (isMounted) {
          setSocialAuthReady({
            google: !!(res.data?.providers?.google ?? res.data?.enabled),
            facebook: !!res.data?.providers?.facebook,
          });
        }
      })
      .catch(() => {
        if (isMounted) setSocialAuthReady({ google: false, facebook: false });
      });
  ...
  const handleSocialLogin = (provider) => {
    if (provider === 'google') {
      if (!socialAuthReady.google) {
        setErrorMessage('Google login is not configured on this deployment yet.');
        return;
      }
      window.location.href = apiUrl('/auth/google');
    } else if (provider === 'facebook') {
      if (!socialAuthReady.facebook) {
        setErrorMessage('Facebook login is not configured on this deployment yet.');
        return;
      }
        ...
        <div className="flex-1 w-full max-w-md mx-auto py-12 px-8 sm:px-4 flex flex-col justify-center">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4">
            <button 
              ...
              disabled={socialAuthReady.google !== true}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition font-bold text-gray-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              {socialAuthReady.google === false ? 'Google Unavailable' : socialAuthReady.google === null ? 'Checking...' : 'Google'}
            </button>
            <button 
              type="button" 
              onClick={() => handleSocialLogin('facebook')}
              disabled={socialAuthReady.facebook !== true}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition font-bold text-gray-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-5 h-5 text-[#1877F2]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
              {socialAuthReady.facebook === false ? 'Facebook Unavailable' : socialAuthReady.facebook === null ? 'Checking...' : 'Facebook'}
            </button>
          </div>
